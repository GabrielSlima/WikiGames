...
def getLastAddedGames():
    try:
        games = session.query(Game, Category).filter(
            Game.category_id == Category.id).limit(10).all()
        return games
    except:
        return None
...
def createGame(
    ...
    try:
        game = Game(
            title=game_title, short_description=game_short_descr,
            long_description=game_long_descr, category_id=game_category_id,
